    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"Please specify a valid number and unit of time (hours/days) script by OWNER- @VirtualVenture..."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = f"ONLY OWNER CAN USE💀OWNER OWNER- @VirtualVenture..."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} OWNER- @VirtualVenture..."
        else:
            response = f"Invalid or expired key buy from OWNER- @VirtualVenture..."
    else:
        response = f"Usage: /redeem <key> if you don't  have  buy from  @VirtualVenture}..."
...
async def allusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        if users:
            ...
            response = f"No data found OWNER- @VirtualVenture..."
    else:
        response = f"ONLY OWNER CAN USE.OWNER- @VirtualVenture..."
    await update.message.reply_text(response)
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 3:
        await update.message.reply_text('Usage: /venture <target_ip> <port> <duration>')
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flooding_command is None:
        await update.message.reply_text('No flooding parameters set. Use /soul to set parameters.')
        return
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    response = f(
        "Welcome to the Flooding Bot by OWNER- @VirtualVenture...! Here are the available commands:\n\n"
        "Admin Commands:\n"
